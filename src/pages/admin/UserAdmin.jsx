...
import { Plus, Pencil, Trash2, UserCog, Shield, Stethoscope, Scissors, Eye, EyeOff } from 'lucide-react'
...
const roleLabels = { admin: 'Administración', vet: 'Doctora', groomer: 'Groomista' }
const roleColors = { admin: 'blue', vet: 'green', groomer: 'purple' }
const roleIcons  = { admin: Shield, vet: Stethoscope, groomer: Scissors }
...
export default function UserAdmin() {
  ...
  if (role !== 'admin') {
    return (
      <div className="max-w-xl mx-auto text-center py-20">
        ...
        <p className="text-gray-500">Solo Administración puede gestionar usuarios.</p>
      </div>
    )
  }
  ...
  function handleSubmit(e) {
    ...
    const data = { username: fd.get('username'), password: fd.get('password'), role: fd.get('role'), name: fd.get('name') }
    ...
    <div className="max-w-3xl mx-auto space-y-5">
            ...
            <Card key={user.id}>
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <Badge variant={roleColors[user.role]} className="mt-1">{roleLabels[user.role]}</Badge>
                </div>
                <div className="flex gap-2 shrink-0">
                  <button onClick={() => openEdit(user)} className="p-2 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors">
                    <Pencil size={15} />
                  </button>
                  {!isMe && (
      ...
      <Modal isOpen={showModal} onClose={() => { setShowModal(false); setEditing(null) }}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <Input label="Nombre completo" name="name" defaultValue={editing?.name} placeholder="Ej. Dra. María López" required />
          <Input label="Usuario" name="username" defaultValue={editing?.username} placeholder="Ej. Doctora" required />
          ...
          <Select label="Rol" name="role" defaultValue={editing?.role || 'vet'}>
          ...
          <div className="flex gap-3 justify-end pt-2">
            ...
            <Button type="submit" icon={Plus}>{editing ? 'Guardar cambios' : 'Crear usuario'}</Button>
          </div>
        </form>
      </Modal>
    </div>
